...
?>
    ...
    <!-- HERO -->
    <div style="padding:72px 40px 64px;border-bottom:1px solid var(--border);position:relative;overflow:hidden;<?php echo has_post_thumbnail() ? 'min-height:360px;display:flex;align-items:flex-end;' : ''; ?>">
        ...
        <div style="position:relative;max-width:700px;">
            ...
            <?php if ($about) : ?>
                <p class="hilife-intro"><?php echo esc_html($about); ?></p>
            <?php endif; ?>
        </div>
    </div>
    ...
    <div style="padding:64px 40px;border-bottom:1px solid var(--border);">
        ...
        <?php foreach ($playlist as $i => $row) : ?>
            <li style="font-size:13px;color:var(--text-dim);padding:8px 0;border-bottom:1px solid var(--border-dim);break-inside:avoid;display:flex;gap:16px;align-items:baseline;">
                <span style="font-size:10px;color:rgba(227,221,88,0.4);font-family:var(--font-mark);min-width:24px;"><?php echo str_pad($i+1, 2, '0', STR_PAD_LEFT); ?></span>
