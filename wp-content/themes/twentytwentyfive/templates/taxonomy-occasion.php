...
$headline    = get_field('occasion_headline', $term);
$intro       = get_field('occasion_intro', $term);
$image_field = get_field('occasion_hero_image', $term);
...
<main class="hilife-main">
    ...
    <div style="padding:64px 40px;border-bottom:1px solid var(--border);">
        ...
        <?php foreach ( $locations as $location ) :
            $loc_image_field = get_field('location_hero_image', $location);
            $loc_image = is_array($loc_image_field) ? $loc_image_field['url'] : $loc_image_field;
            $intersection_url = '/' . $term->slug . '/' . $location->slug;
        ?>
            <a href="<?php echo esc_url($intersection_url); ?>"
               ...
               onmouseout="this.style.borderColor='var(--border)';var i=this.querySelector('.card-img');if(i)i.style.filter='brightness(0.75)';this.querySelector('.card-title').style.color='var(--text-dim)'">
                <?php if ( $loc_image ) : ?>
                    <div class="card-img" style="position:absolute;inset:0;background-image:url('<?php echo esc_url($loc_image); ?>');background-size:cover;background-position:center;filter:brightness(0.75);transition:filter 0.4s;"></div>
                <?php else : ?>
    ...
    <!-- RECENT EVENTS -->
    <div style="padding:64px 40px;border-bottom:1px solid var(--border);">
        <div class="hilife-section-label">Recent <?php echo esc_html($term->name); ?></div>
